...
import { ArrowLeft, ChevronDown, ChevronRight, Video, Users, TrendingUp } from 'lucide-react'
...
export default function StaticVMRDetailPage() {
  const params = useParams()
  const [vmr, setVmr] = useState<StaticVMRDetail | null>(null)
  const [loading, setLoading] = useState(true)
  const [expandedConf, setExpandedConf] = useState<number | null>(null)

  useEffect(() => {
    const name = params.name as string
    fetch(`/api/vmrs/static/${encodeURIComponent(name)}`)
      .then(r => r.json())
      .then(setVmr)
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <StatsCard title="Total Conferences" value={vmr.totalConferences} icon={Video} color="blue" />
        <StatsCard title="Total Participants" value={vmr.totalParticipants} icon={Users} color="green" />
        <StatsCard title="Avg Participants" value={vmr.avgParticipants} subtitle="Per conference" icon={TrendingUp} color="purple" />
      </div>
