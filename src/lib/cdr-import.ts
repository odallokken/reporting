...
export interface PexipCDRConference {
  id?: string
  name?: string
  start_time?: string
  end_time?: string
...
export interface PexipCDRParticipant {
  display_name?: string
  call_uuid?: string
  connect_time?: string
...
export interface RunCdrImportOptions {
  baseUrl: string
  username: string
  password: string
  minDurationSeconds?: number
  /**
   * Logger source tag. Defaults to `cdr-import`. Callers may override this
   * (for example the clean-import route uses `cdr-clean-import`) so log
   * entries are easy to attribute to the action that triggered them.
   */
  logSource?: string
}
...
export interface CdrImportError {
  error: string
  status: number
}

export type CdrImportOutcome =
  | { ok: true; result: CdrImportResult }
  | { ok: false; error: CdrImportError }
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(url, {
      ...
      if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        resolve(fetchWithBasicAuth(redirectUrl, username, password, logSource))
        return
      }
...
/**
 * Performs a CDR import from a Pexip Management Node into the local database.
 *
 * This is the shared implementation used by both `/api/cdrs/import` (additive)
 * and `/api/cdrs/clean-import` (destructive replace). It is purely additive
 * with respect to the database — callers that want a clean slate are
 * responsible for wiping CDR tables before invoking this function.
 */
export async function runCdrImport(options: RunCdrImportOptions): Promise<CdrImportOutcome> {
  const logSource = options.logSource ?? 'cdr-import'
  const baseUrl = options.baseUrl?.trim()
  const username = options.username?.trim()
  const password = options.password ?? ''
  const minDurationSeconds = typeof options.minDurationSeconds === 'number' && options.minDurationSeconds > 0
    ? options.minDurationSeconds
    : 0

  if (!baseUrl || !username || !password) {
    return { ok: false, error: { error: 'Missing required fields', status: 400 } }
  }
  ...
  try {
    ...
    await log('error', 'Invalid Management Node URL', { source: logSource, details: `Provided URL: ${baseUrl}` })
    return {
      ok: false,
      error: {
  ...
  try {
    response = await fetchWithBasicAuth(url, username, password, logSource)
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
    await log('error', 'Network error contacting Pexip Management Node', { source: logSource, details: message })
    return {
      ok: false,
      error: {
  ...
  if (!response.ok) {
    ...
    await log('error', `Pexip API returned ${response.status}`, { source: logSource, details })
    return {
      ok: false,
      error: { error: `Pexip API returned ${response.status}.${guidance}`, status: 502 },
    }
  }
  ...
  while (nextUrl) {
    const pageResponse = nextUrl === url
      ? response
      : await fetchWithBasicAuth(nextUrl, username, password, logSource)

    if (!pageResponse.ok) {
      await log('warn', `Pexip API returned ${pageResponse.status} while fetching page`, { source: logSource, details: `URL: ${nextUrl}` })
      break
    }
  ...
  await log('info', `Fetched ${conferences.length} conferences from Pexip API`, { source: logSource })
  ...
  for (const conf of conferences) {
    try {
      ...
      if (conf.id) {
        ...
        while (partNextUrl) {
          try {
            const partResponse = await fetchWithBasicAuth(partNextUrl, username, password, logSource)
            if (!partResponse.ok) {
              await log('warn', `Failed to fetch participants for conference ${conf.id}: HTTP ${partResponse.status}`, { source: logSource })
              break
            }
            ...
            await log('warn', `Error fetching participants for conference ${conf.id}`, { source: logSource, details: msg })
            break
          }
        }
      }
      ...
      await log('warn', `Failed to import conference: ${conf.call_id ?? conf.name ?? 'unknown'}`, { source: logSource, details: errMessage })
      skipped++
    }
  }

  await log('info', `CDR import complete: ${imported} imported, ${skipped} skipped`, { source: logSource })

  return { ok: true, result: { imported, skipped, total: conferences.length } }
}
